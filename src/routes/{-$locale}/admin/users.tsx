...
function UsersPage() {
  ...
  return (
    <AppShell user={{ name: session?.user?.name, email: session?.user?.email ?? '', role: session?.user?.role ?? 'admin', image: session?.user?.image ?? null }} isPro={false} active="admin-users">
      <div className="mb-5">
