...
模板默认就带的安全措施。报告漏洞请看根目录 [`SECURITY.md`](https://github.com/kohunglee/daka/blob/main/SECURITY.md);这里讲**已经就位的防护**和怎么调。
...
外加**仅生产**注入的 `Content-Security-Policy`(dev 不注入,因为 Vite 需要 eval/ws)。当前策略允许任意 HTTPS 外部脚本、样式、字体、图片、接口、WebSocket、iframe、媒体和 Worker，便于 666 模式中的自定义 Head/Footer HTML 接入第三方服务；同时保留 `frame-ancestors 'none'`、`object-src 'none'`、`base-uri 'self'` 等基础限制。由于自定义 HTML 会在全站执行，只能粘贴完全信任的代码。

改 CSP:编辑 `security-headers.ts` 里的 `PROD_CSP`。如果未来需要进一步收紧，再把 `https:` 改回具体的第三方域名白名单，并逐项验证站点功能。
